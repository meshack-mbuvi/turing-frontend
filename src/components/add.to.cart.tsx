                        ...
                        <div className="card pb-4 card-full">
                            <Form title="" handleSubmit={(e: any) => this.handleSubmit(e)}>
                                <div className="row">
                                    ...
                                    </div>{error ? <div className="col-md-8 m-auto text-danger" ><p >{error}</p></div> : ''}
                                    {attributeTypes ? attributeTypes.map((attributeType, index) => {
                                        return <div className="col-md-8 m-auto" key={index}>

                                            <div className="form-group">
                                                <label>{attributeType}</label>
                                                <select className="form-control"
                                                    onChange={(e) => this.handleOnChange(e)}
                                                    id={attributeType.toString()}>
                                                    <option value='0'>Please select {attributeType}</option>
                                                    {productAttributes.map((attribute: any, index1: any) => {
                                                        return attribute.attribute_name === attributeType ? <option value={attribute.attribute_value_value} key={index1}>{attribute.attribute_value}</option> : ''
                                ...
                                <div className="form-group text-center">
                                    <Button type="submit" value="Add to cart" className="bg mb-4" />
                                </div>
